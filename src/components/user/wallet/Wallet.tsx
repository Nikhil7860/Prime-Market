...
import { depositMoney, getWalletBalance, withdrawMoney } from "@/services/wallet.service";
...
export default function UserWallet() {
    ...
    const handleDepositSubmit = async () => {
        await depositMoney({ amount: parseInt(amount), userId: userDetails._id })
        await fetchWalletBalance(userDetails._id)
        await fetchTransactions(userDetails?._id)
        setWalletOpen(false)
        toast.success("Deposit Successful 🎉");
    };


    const handleWithdrwalSubmit = async () => {
        let response: any = await withdrawMoney({ amount: parseInt(amount), userId: userDetails._id })
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                    ...
                    <div className="w-full max-w-md rounded-3xl bg-slate-900 p-8">
                        ...
                        <div className="mb-6 flex items-center justify-between">
                            ...
                            <button onClick={(() => setWalletOpen(false))}><X className="text-white" /></button>
                        ...
                        <button onClick={handleDepositSubmit} className="w-full rounded-xl bg-blue-600 py-3 font-semibold text-white transition hover:bg-blue-700">
                            Add Money
                        </button>
